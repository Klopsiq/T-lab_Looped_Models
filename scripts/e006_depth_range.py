"""E006: test whether useful depth follows the trained depth range."""
...
def registration(args, arm, seed, manifest):
    ...
    paths = [
        ...
        root / "research/protocols/E006_depth_range.md", root / args.data / "manifest.json",
...
def train(args, arm_name, seed, manifest, validation, validation_ids):
    ...
    elapsed = prior_seconds + time.perf_counter() - started
    result = {
        "experiment": "E006", "arm": arm_name, "seed": seed, "parameters": model.parameter_count(),
...
def main():
    ...
    parser.add_argument("--output", default="runs/E006")
    ...
    if not torch.cuda.is_available() or not args.device.startswith("cuda"):
        raise SystemExit("E006 requires CUDA")
    ...
    if manifest["context"] != args.context or manifest["tokenizer"]["vocab_size"] != 8192:
        raise ValueError("E006 corpus does not match registered context/vocabulary")
