"""E007: replicate the E006 loss comparison on held-out training seeds."""
...
ARMS = {
    "combined_random8_24_final": dict(relative=True, depth_rope=True, min_depth=8, max_depth=24, auxiliary=False),
    "combined_random8_24_aux": dict(relative=True, depth_rope=True, min_depth=8, max_depth=24, auxiliary=True),
}
...
def registration(args, arm, seed, manifest):
    ...
    paths = [
        ...
        root / "research/protocols/E007_seed_replication.md", root / args.data / "manifest.json",
...
def train(args, arm_name, seed, manifest, validation, validation_ids):
    ...
    cfg = ModelConfig(vocab_size=manifest["tokenizer"]["vocab_size"], width=512, intermediate=1280,
                      heads=8, kv_heads=2, core_layers=2, relative=arm["relative"], depth_rope=arm["depth_rope"])
    ...
    with (directory / "learning_curve.jsonl").open("a") as log:
        for step in range(start_step, target_steps):
            ...
            loops = int(torch.randint(arm["min_depth"], arm["max_depth"] + 1, (1,), generator=schedule_rng))
    ...
    elapsed = prior_seconds + time.perf_counter() - started
    result = {
        "experiment": "E007", "arm": arm_name, "seed": seed, "parameters": model.parameter_count(),
...
def main():
    ...
    parser.add_argument("--output", default="runs/E007")
    ...
    if not torch.cuda.is_available() or not args.device.startswith("cuda"):
        raise SystemExit("E007 requires CUDA")
    ...
    if manifest["context"] != args.context or manifest["tokenizer"]["vocab_size"] != 8192:
        raise ValueError("E007 corpus does not match registered context/vocabulary")
